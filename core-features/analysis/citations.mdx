...
<Latex>{"\\text{Citation Rate} = \\frac{\\text{Responses with your URL}}{\\text{Total Chats}} \\times 100"}</Latex>

**Average citations** — How many times your URL (or a source) is cited per response:

<Latex>{"\\text{Avg Citations} = \\frac{\\text{Total Citations}}{\\text{Responses}}"}</Latex>
